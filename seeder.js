...
const subjects_list = JSON.parse(
  fs.readFileSync(`${__dirname}/./data/subjects.json`)
);
const students_list = JSON.parse(
...
async function seedWithDummyData() {
  try {
    ...
    console.log(
      `${createdStudents.length} Students and ${createdSubjects.length} Subjects created.`
    );
  } catch (error) {
